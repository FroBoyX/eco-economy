# Ironwood Paper Milling — Current Steam-Era Table

Rebuilt from the supplied Eco 14 Core using current Natural Fiber and Painting inputs.

Paper Milling is Tier 3 and is inside the current Steam-entry economy. Current Core explicitly tags Kelp and several other gathered plants as `NaturalFiber`, so Cellulose Fiber and Paper do not require future Wood Pulp or petroleum chemistry.

> The current Exchange/schema ledger is authoritative where older explanatory calculations in this profession note differ. **Waxed Paper is current-era:** Paraffin is available from the Milling 4 `Wax` recipe at the Mill.

## Cellulose Fiber — Paper Milling 1

Core:

- 12 `NaturalFiber`
- 60 calories
- -> 1 Cellulose Fiber

Kelp is a valid current Natural Fiber and sells publicly for 0.04. At Paper Milling 1:

- 9.6 Natural Fiber × 0.04 = 0.384
- 48 calories = 0.048
- cash cost = 0.432

**Cellulose Fiber: 0.48 Town Buy / 0.50 Town Sell.**

## Paper — Paper Milling 1

Core:

- 2 Cellulose Fiber
- 20 calories
- -> 6 Paper

At level 1:

- 1.6 Cellulose Fiber × 0.50 = 0.800
- 16 calories = 0.016
- cash cost/Paper = 0.136

**Paper: 0.15 / 0.16.**

## Waxed Paper — Paper Milling 2

Core:

- 10 Paper
- 1 Paraffin
- 180 calories
- -> 4 Waxed Paper
- workstation: **Small Paper Machine**

At Paper Milling 2 the multiplier is 0.75. Under the integrated-workshop rule, Paper is a same-Paper-Milling component and enters at its current **0.1179 Projected Cost**. Paraffin crosses from Milling and enters at **0.58 Town Sell**.

- 7.5 Paper × 0.1179 = 0.88425
- 0.75 Paraffin × 0.58 = 0.435
- 135 calories = 0.135
- batch cost = 1.45425
- /4 = **0.3636 projected cost per Waxed Paper**

As a current research/progression intermediate:

**Waxed Paper: 0.42 Town Buy / 0.44 Town Sell.**

This directly unlocks the current material chain needed for Dendrology Research Paper Modern. A future Oil Drilling route can reduce Paraffin cost later, but Waxed Paper itself is not petroleum-gated.

## Printing Supplies — Paper Milling 1

Painting now provides current-era Ink at **1.19 / 1.25**.

Core:

- 2 Ink
- 20 Paper
- 360 calories
- -> 1 Printing Supplies

At Paper Milling 1:

- 1.6 Ink × 1.25 = 2.000
- 16 Paper × 0.16 = 2.560
- 288 calories = 0.288
- cash cost = **4.848**

As a multi-step printing intermediate:

**Printing Supplies: 5.72 Town Buy / 6.01 Town Sell.**

## Posters — Paper Milling 4, Printing Press

Redwoods Poster is the parent recipe family; Cart Blueprint, Election and Periodic Table posters are tag-product variants with the same ingredient burden.

Current effective recipe at Paper Milling 4:

- 1.3 Printing Supplies
- 6.5 Paper
- 19.5 calories
- -> 1 poster

Cash cost:

- 1.3 × 6.01 = 7.813
- 6.5 × 0.16 = 1.040
- labor ≈ 0.020
- total ≈ **8.873**

**Current posters: 10.20 Town Buy / 10.71 Town Sell each.**

This applies to:

- Redwoods Poster;
- Cart Blueprint Poster;
- Election Poster;
- Periodic Table Poster.

## Paper Milling Upgrade — level 2

Core:

- 32 Paper, static
- 6,000 calories

At level 2:

- 32 × 0.16 = 5.120
- 4,500 calories = 4.500
- cash cost = 9.620

As a capital/skill upgrade:

**Paper Milling Upgrade: 12.51 / 13.14.**

## Paper Lanterns — Paper Milling 1

### Small Paper Lantern

Core: 10 Paper + 4 Cotton Fabric + 1 static Light Bulb + 60 calories.

Current cash cost = **17.178**.

**Small Paper Lantern: 19.75 / 20.74.**

### Large Paper Lantern / Large Festive Paper Lantern

Core: 20 Paper + 8 Cotton Fabric + 1 static Light Bulb + 60 calories.

Current cash cost = **21.338**.

**Large Paper Lantern: 24.54 / 25.77.**

**Large Festive Paper Lantern: 24.54 / 25.77.**

## Current progression status

- Waxed Paper is **current**, using Milling-produced Paraffin;
- Dendrology Research Paper Modern is therefore reachable through the current Paraffin/Waxed Paper material chain once its research prerequisites and Laboratory are available;
- future Oil Drilling changes the efficient Paraffin route, not whether Paraffin/Waxed Paper exists in the current progression.

## Exchange-ready Paper Milling table

| Item | Town Buys | Town Sells |
|---|---:|---:|
| Cellulose Fiber | **0.48** | **0.50** |
| Paper | **0.15** | **0.16** |
| **Waxed Paper** | **0.42** | **0.44** |
| Printing Supplies | **5.72** | **6.01** |
| Redwoods Poster | **10.20** | **10.71** |
| Cart Blueprint Poster | **10.20** | **10.71** |
| Election Poster | **10.20** | **10.71** |
| Periodic Table Poster | **10.20** | **10.71** |
| Paper Milling Upgrade | **12.51** | **13.14** |
| Small Paper Lantern | **19.75** | **20.74** |
| Large Paper Lantern | **24.54** | **25.77** |
| Large Festive Paper Lantern | **24.54** | **25.77** |
